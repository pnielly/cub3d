...
/*   Updated: 2020/01/26 20:50:29 by pnielly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_ray_dir(t_stock *stock, int c)
{
	...
	if (stock->ray->x_or_y == 'x')
		stock->ray->dist_wall = fabs((mapx - stock->player->pos_x + (1 - stock->ray->step_x) / 2) / stock->ray->dir->x);
	else
		stock->ray->dist_wall = fabs((mapy - stock->player->pos_y + (1 - stock->ray->step_y) / 2) / stock->ray->dir->y);
	/** Calculate height of the wall**/
	stock->wall_height = fabs(stock->R->y / stock->ray->dist_wall);
