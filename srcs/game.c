...
/*   Updated: 2020/02/25 18:34:23 by pnielly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_choose_level(t_stock *stock)
{
	...
	ft_put_string(stock, WELCOME, stock->r->x / 6, stock->r->y / 6, PINK);
	ft_put_string(stock, KINDER, stock->r->x / 6, stock->r->y / 4, PINK);
	ft_put_string(stock, LEGEND, stock->r->x / 6, stock->r->y / 2, PINK);
...
int		ft_obj(t_stock *stock)
{
	mlx_hook(stock->win_ptr, DESTROYNOTIFY,
			STRUCTURENOTIFYMASK, &ft_win_exit, stock);
	ft_put_string(stock, OBJ, stock->r->x / 6, stock->r->y / 6, PINK);
	ft_put_string(stock, SUITE, stock->r->x / 6, stock->r->y / 4, PINK);
	mlx_hook(stock->win_ptr, KEYPRESS, KEYPRESSMASK, ft_level, stock);
	mlx_loop(stock->mlx_ptr);
	return (0);
}
