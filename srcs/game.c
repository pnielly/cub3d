...
/*   Updated: 2020/03/11 11:09:55 by pnielly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_resolution(t_stock *stock)
{
	...
	if (stock->r->y < 0)
		return (ft_putstr("MAP RESOLUTION = 0\n"));
	if (ft_max(stock->r->y, stock->r->x) > 1000)
		stock->speed_coef = ft_max(stock->r->y, stock->r->x);
	else
		stock->speed_coef = 1000;
	stock->speed = SPEED * stock->speed_coef / 1000;
	return (0);
}
