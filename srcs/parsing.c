...
/*   Updated: 2020/03/03 14:26:39 by pnielly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (i * j == 0 || i == (int)ft_strlen(stock->map->tab[j]) - 1 || j == stock->map->nb_lines)
		if (stock->map->tab[j][i] != '1')
			return (ft_putstr("MAP ISN'T SURROUNDED BY WALLS\n"));
	if (j && stock->map->diff > 0 && ft_walled(stock, j))
		return (ft_putstr("MAP ISN'T SURROUNDED BY WALLS\n"));
	if (stock->map->tab[j][i] && !ft_strchr("01234NSEW", stock->map->tab[j][i]))
	{
		printf("i = %d, j = %d, tab[j][i] = <%c>\n", i, j, stock->map->tab[j][i]);
...
/**
void		ft_fill_map(t_stock *stock)
{
	int	i;
	int	j;
