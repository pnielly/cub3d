...
/*   Updated: 2020/02/25 17:32:05 by pnielly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_build_screen(t_stock *stock)
{
	...
	if (stock->lost || stock->won)
	{
		...
		if (stock->lost)
		{
			mlx_string_put(stock->mlx_ptr, stock->win_ptr, stock->r->x / 2, stock->r->y / 2, PINK, GAMEOVER);
			if (stock->k)
				mlx_string_put(stock->mlx_ptr, stock->win_ptr, 10, stock->r->y / 2 + 50, PINK, TIPKINDER);
			if (!stock->k)
				mlx_string_put(stock->mlx_ptr, stock->win_ptr, 10, stock->r->y / 2 + 50, PINK, TIPLEGEND);
		}
		else
			mlx_string_put(stock->mlx_ptr, stock->win_ptr, stock->r->x / 2, stock->r->y / 2, PINK, VICTORY);
